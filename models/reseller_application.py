...
class ResellerApplication(models.Model):
    ...
    'view_mode': 'form',
    'target': 'current',
    'context': dict(self.env.context, show_approval_msg=True)
}
    ...
    def _create_portal_user(self, partner):
        ...
        if existing_user:
            if existing_user.partner_id != partner:
                existing_user.partner_id = partner
        ...
        user = self.env["res.users"].sudo().create(user_data)
